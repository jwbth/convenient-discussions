...
function main() {
      ...
      for (let i = 0; i < cd.msgs.length; i++) {
        ...
        if (prevMsgId !== undefined) {
          ...
          if (proportion < 0 || proportion >= 1) {
            console.warn('Пропорция не должна быть меньше 0 или больше или равна 1.', proportion, currentMsgId,
              prevMsgId, viewportTop, viewportBottom);
          }
          nextMsgId = Math.round((Math.abs(currentMsgId - prevMsgId) - 1) * proportion +
            Math.min(prevMsgId, currentMsgId) + 0.5
          );
          if (changedDirection) {
            // If the convergence goes by a scheme: 1 – 10 – 5 – 3 (two times down), then the values of prev...
            // should be saved to use 1 and 5 for getting 3, not 5 and 10.
  ...
  const generateAnyCasePattern = string => {
    let result = '';
  ...
  // Generating a regexp from a config value. Only the part generated below is case-sensitive, this
  // is why we generate it this way.
  ...
  for (let i = 0; i < cd.config.USER_NAMESPACES.length; i++) {
    ...
    userNamePattern += generateAnyCasePattern(cd.config.USER_NAMESPACES[i]);
    if (i === cd.config.USER_NAMESPACES.length - 1) {
      userNamePattern += ')';
    }
  }
  userNamePattern += '[ _]*:[ _]*|(?:[Ss][Pp][Ee][Cc][Ii][Aa][Ll][ _]*:[ _]*[Cc][Oo][Nn][Tt][Rr]' +
    '[Ii][Bb][Uu][Tt][Ii][Oo][Nn][Ss]|';
  userNamePattern += generateAnyCasePattern(
    cd.config.SPECIAL_CONTRIBUTIONS_PAGE.replace(':', ' : ')
  );
  userNamePattern += ')\\/[ _]*)';
