 ...
 * complicated).
 *
 * @param {string} languageType `'content'` or `'user'`.
 ...
 * @license MIT
 */
function getTimestampMainPartPattern(languageType) {
  const isContentLanguage = languageType === 'content';
...
/**
 * Create and set the regexps that match timestamps in a local date format.
 *
 * This calls `getTimestampMainPartPattern()`.
 *
 * @param {string} languageType `'content'` or `'user'`.
 * @private
 */
function setTimestampRegexps(languageType) {
  if (languageType === 'content') {
...
/**
 * Set the properties of the global object containing codes of date components for the function that
 * parses timestamps in the local date format based on the result of matching the regexp set by
 * `setTimestampRegexps()`.
 *
 * @param {string} languageType `'content'` or `'user'`.
 * @private
 */
function setMatchingGroupsForTimestampParser(languageType) {
  if (languageType === 'content') {
    /**
 ...
 */
export function initTimestampParsingTools(languageType) {
  setTimestampRegexps(languageType);
  setMatchingGroupsForTimestampParser(languageType);
}
...
export function parseTimestamp(timestamp, timezone) {
  ...
  if (!parseTimestampContentRegexp) {
    parseTimestampContentRegexp = new RegExp(
      `^([^]*)(${cd.g.CONTENT_TIMESTAMP_REGEXP.source})(?!["»])`
    );
    parseTimestampUiRegexp = new RegExp(`^([^]*)(${cd.g.UI_TIMESTAMP_REGEXP.source})`);
  }

  const regexp = timezone === undefined ? parseTimestampContentRegexp : parseTimestampUiRegexp;
