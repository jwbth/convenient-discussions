class TributeRange {
        ...
        if (effectiveRange !== undefined && effectiveRange !== null) {
            ...
            let inputOk = (mostRecentTriggerCharPos >= 0 &&
                (
                    ...
                    convenientDiscussions.g.PUNCTUATION_REGEXP.test(
                        effectiveRange[mostRecentTriggerCharPos - 1]
                    )
                )
            )
            if (inputOk) {
    ...
    lastIndexWithLeadingSpace(str, trigger) {
        ...
        for (let cidx = 0, len = str.length; cidx < len; cidx++) {
            ...
            let leadingSpace = convenientDiscussions.g.PUNCTUATION_REGEXP.test(reversedStr[cidx + 1])
